...
def artifact(kind: str, *, node_id: str = "S1-root", sha: str = "b" * 64) -> dict:
    return {
        ...
        "created_at": "2026-07-28T00:00:00Z", "generator": "fixture", "status": "PASS" if kind == "mocktest" else "COMPLETED",
...
class LeafAdapterTests(unittest.TestCase):
    def test_missing_any_formal_input_is_error(self):
        values = {kind: artifact(kind) for kind in ("prd", "architecture", "testcases", "mocktest")}
        values["mocktest_report"] = values.pop("mocktest")
    ...
    def test_leaf_is_only_eligible_after_complete_pass_and_verified_hashes(self):
        ...
        self.assertFalse(result["formal_artifact"])
