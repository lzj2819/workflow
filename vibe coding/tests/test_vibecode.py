...
class LeafDecisionTests(unittest.TestCase):
    def test_public_and_legacy_decisions_normalize(self) -> None:
        expected = {
    ...
    def test_scanner_admits_only_stop_layering(self) -> None:
        ...
        self.assertEqual({leaf["node_id"] for leaf in leaves}, {"stop", "legacy"})
        self.assertTrue(all(leaf["decision"] == "STOP_LAYERING" for leaf in leaves))

    @staticmethod
    def _write_node(node: Path, decision: str) -> None:
        (node / "architecture" / "output").mkdir(parents=True)
        (node / "features").mkdir()
        (node / "leaf-gate.report.json").write_text(
            json.dumps({"decision": decision}), encoding="utf-8"
        )
        (node / "prd.md").write_text("# PRD\n", encoding="utf-8")
        (node / "traceability.md").write_text("# Traceability\n", encoding="utf-8")
        (node / "risks.md").write_text("# Risks\n", encoding="utf-8")
        (node / "architecture" / "output" / "06-interface-contracts.md").write_text(
            "# Contract\n", encoding="utf-8"
        )
        (node / "features" / "acceptance.feature").write_text(
            "Feature: Acceptance\n", encoding="utf-8"
        )
...
class SchemaTests(unittest.TestCase):
    def test_baseline_schemas_are_valid_json_and_share_public_statuses(self) -> None:
        ...
        self.assertTrue(
            set(VIBECODE.LEAF_DECISION_ALIASES.values()).issubset(status_enum)
        )
...
class RunGraphTests(unittest.TestCase):
    ...
    def test_two_level_continue_is_idempotent_and_parent_waits(self) -> None:
        ...
        children = [{"node_id": "child", "parent_node_id": "root"}]
    ...
    def test_invalid_child_batch_does_not_partially_schedule(self) -> None:
        ...
        with self.assertRaisesRegex(ORCHESTRATOR.GraphError, "requirement_ids"):
            ORCHESTRATOR.apply_leaf_decision(
                ...
                [
                    {"node_id": "valid"},
                    {"node_id": "invalid", "requirement_ids": "not-a-list"},
                ],
            )
