...
LEAF_DECISION_ALIASES = {
    "CONTINUE_LAYERING": "CONTINUE_LAYERING",
    "STOP_LAYERING": "STOP_LAYERING",
    "ERROR": "ERROR",
    "LEAF_READY": "STOP_LAYERING",
    "DONE_LAYERING": "STOP_LAYERING",
    "NEEDS_DECOMPOSITION": "CONTINUE_LAYERING",
    # Fail closed: these legacy decisions require work before coding can start.
    "NEEDS_SPEC_REFINEMENT": "ERROR",
    "HUMAN_REVIEW": "ERROR",
}
...
REQUIRED_LEAF_FILES = {
    ...
    "risks": ["risks.md"],
    "leaf_gate_report": ["leaf-gate.report.json"],
}
...
def first_existing(node_dir: Path, names: list[str]) -> Path | None:
    for name in names:
        path = node_dir / name
        if path.exists():
            return path
...
def find_arch_dir(node_dir: Path) -> Path | None:
    candidates = [
        node_dir / "architecture" / "output",
        node_dir / "architecture",
        node_dir / "arch" / "output",
        node_dir / "arch",
    ]
...
def find_contract(node_dir: Path) -> Path | None:
    for base in [node_dir / "architecture" / "output", node_dir / "architecture", node_dir / "arch" / "output", node_dir / "arch", node_dir]:
        if not base.exists():
            continue
        exact = base / "06-interface-contracts.md"
        if exact.exists():
            return exact
        matches = sorted(base.glob("*contract*.md"))
        if matches:
            return matches[0]
    return None


def find_features(node_dir: Path) -> list[Path]:
    candidates = list((node_dir / "features").glob("*.feature")) if (node_dir / "features").is_dir() else []
    candidates.extend(node_dir.glob("*.feature"))
...
def normalize_leaf_decision(value: Any) -> str | None:
    if not isinstance(value, str):
        return None
    return LEAF_DECISION_ALIASES.get(value.strip().upper())


def find_decision(value: Any) -> str | None:
    normalized = normalize_leaf_decision(value)
    if normalized:
        return normalized
    if isinstance(value, dict):
        for key in ("decision", "leaf_gate_decision", "verdict", "result", "status"):
            if key in value:
                found = find_decision(value[key])
                if found:
                    return found
        for child in value.values():
            found = find_decision(child)
            if found:
                return found
...
def scan_leaves(leaf_root: Path) -> list[dict[str, Any]]:
    ...
    for report in sorted(leaf_root.rglob("leaf-gate.report.json")):
        node_dir = report.parent
        try:
            decision = find_decision(json.loads(report.read_text(encoding="utf-8")))
        except json.JSONDecodeError:
            decision = None
        if decision != "STOP_LAYERING":
            continue

        node_id = node_id_from_dir(node_dir, leaf_root)
        arch = find_arch_dir(node_dir)
        contract = find_contract(node_dir)
        features = find_features(node_dir)
        missing = []
        for label, names in REQUIRED_LEAF_FILES.items():
            if not first_existing(node_dir, names):
        ...
        leaves.append(
            {
                ...
                "decision": decision,
                "prd": rel(first_existing(node_dir, ["prd.md", "PRD.md"])) if first_existing(node_dir, ["prd.md", "PRD.md"]) else None,
                "architecture": rel(arch) if arch else None,
                "contract": rel(contract) if contract else None,
                "features": [rel(path) for path in features],
                "traceability": rel(first_existing(node_dir, ["traceability.md"])) if first_existing(node_dir, ["traceability.md"]) else None,
                "risks": rel(first_existing(node_dir, ["risks.md"])) if first_existing(node_dir, ["risks.md"]) else None,
                "owner_decision": rel(first_existing(node_dir, ["owner-decision.md"])) if first_existing(node_dir, ["owner-decision.md"]) else None,
                "missing": missing,
                "wave": 1,
                "status": "DISCOVERED" if not missing else "INPUT_INCOMPLETE",
            }
        )
    return leaves
...
def integration_map_markdown(leaves: list[dict[str, Any]]) -> str:
    ...
    for parent, children in sorted(groups.items()):
        ...
        lines.append("")
    if not groups:
        lines.append("No `STOP_LAYERING` nodes found (legacy `LEAF_READY` is accepted).")
    return "\n".join(lines)


def contract_index_markdown(leaves: list[dict[str, Any]]) -> str:
    lines = ["# Global Contract Index", "", "| Node | Contract | Features |", "| --- | --- | --- |"]
    for leaf in leaves:
        features = "<br>".join(f"`{item}`" for item in leaf.get("features", [])) or "-"
        contract = f"`{leaf['contract']}`" if leaf.get("contract") else "-"
        lines.append(f"| `{leaf['node_id']}` | {contract} | {features} |")
    if not leaves:
        lines.append("| - | - | - |")
    return "\n".join(lines)
...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    for leaf in leaves:
        ...
        report.append("")
    if not leaves:
        report.append("No `STOP_LAYERING` nodes were found (legacy `LEAF_READY` is accepted).")
...
def leaf_values(leaf: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "traceability": leaf.get("traceability") or "",
        "risks": leaf.get("risks") or "",
        "owner_decision": leaf.get("owner_decision") or "-",
...
def cmd_self_test(args: argparse.Namespace) -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (base / "architecture" / "output").mkdir(parents=True)
        (base / "features").mkdir()
        write_text(base / "leaf-gate.report.json", json.dumps({"decision": "LEAF_READY"}))
        write_text(base / "prd.md", "# PRD\n")
        write_text(base / "architecture" / "output" / "06-interface-contracts.md", "# Contract\n\nGET /demo\n")
        write_text(base / "features" / "demo.feature", "Feature: Demo\n")
        write_text(base / "traceability.md", "# Traceability\n")
        write_text(base / "risks.md", "# Risks\n")
        leaves = scan_leaves(Path(tmp) / "workspace" / "nodes")
        assert len(leaves) == 1, leaves
        assert leaves[0]["status"] == "DISCOVERED", leaves[0]
        assert find_decision({"x": [{"decision": "LEAF_READY"}]}) == "STOP_LAYERING"
        assert find_decision({"decision": "CONTINUE_LAYERING"}) == "CONTINUE_LAYERING"
        assert find_decision({"status": "ERROR"}) == "ERROR"
