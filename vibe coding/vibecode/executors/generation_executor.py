...
_SPECS = {
    ...
    "architecture": ("architecture.md", "architecture", "Create Markdown architecture with named components, entry contract, flows, error behavior, and explicit interfaces. For strict validation, include a `validate-arch-package` JSON comment; a `Component registry` table with `child_id`, `responsibility`, and `dispatch_kind` columns (use dispatch_kind `component`); and a second `组件职责` table with `Component` and `Responsibility` columns. Name the public entry component exactly `Public API Service`. Add `Entry endpoint and request`, `Request flow`, and an actual Mermaid `sequenceDiagram` beginning `Client->>PublicAPIService:`. Add an `Internal contract mapping` table whose columns are `contract_id`, `Owner → Consumer`, `触发与 schema`, and `Errors, idempotency, compatibility`; every internal flow must have a Provider/Consumer contract with explicit 输入 and 输出 fields. Then add a `Contract mapping` section with the same public endpoint's contract_id, Provider, Consumer, explicit inputs, outputs, and errors."),
