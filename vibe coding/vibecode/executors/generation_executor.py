...
_SPECS = {
    ...
    "architecture": ("architecture.md", "architecture", "Create Markdown architecture with named components, entry contract, flows, error behavior, and explicit interfaces. For strict validation, include a `validate-arch-package` JSON comment; a `Component registry` table with `child_id`, `responsibility`, and `dispatch_kind` columns (use dispatch_kind `component`); and a second `组件职责` table with `Component` and `Responsibility` columns. The public entry's canonical child_id must be `public-api-service`; use `Public API Service` only as its human-readable display name. In the `组件职责` table specifically, the `Component` cell must be exactly the bare canonical child_id `public-api-service`, without a display name, backticks, or parentheses. Add `Entry endpoint and request`, `Request flow`, and an actual Mermaid `sequenceDiagram` beginning `Client -> PublicAPIService:`. Add an `Internal contract mapping` table whose columns are exactly `contract_id`, `Owner → Consumer`, `触发与 schema`, and `Errors, idempotency, compatibility`. In every Owner/Consumer cell use only bare canonical child_ids separated by `→` (never `Provider:`, `Consumer:`, or prose). Every strict component must appear on at least one scenario-reachable flow; omit components not used by the Feature. A contract must never name the same internal child_id as both provider and consumer: use only distinct component edges, or external `health-client → public-api-service` / `public-api-service → health-client` boundaries for a one-component path. Declare a machine-readable inbound public contract and a public response contract. For every contract, write fields as backtick lists immediately after `输入:` and `输出:` (for example, `输入: `event`; 输出: `status_code, body``). For a one-component public health path, both the inbound and outbound external boundaries must use `输入: `event`; 输出: `status_code, body``: never declare `status_code` or `body` as an input to `public-api-service`. To make the inbound contract parser-visible, include exactly one `### GET /health` subsection: directly under that heading place `**输入**` and a JSON code block whose top-level required key is `event`, then `**输出**` and a response JSON code block. Do not put a JSON-only contract block before this parser-visible input/output pair. Then add a `Contract mapping` section with the same public endpoint's contract_id, Provider, Consumer, explicit inputs, outputs, and errors."),
