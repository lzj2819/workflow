...
> 日期：2026-07-28；实施状态更新：2026-07-29  
> 范围：实施、实验和论文协作计划；计划中的 P0 只有在相应 Gate 与证据通过后才可标记完成。  
...
### 0.1 实施状态更新（2026-07-29）

- 实施代码和最新运行证据位于隔离的 `.coord-worktree/`；根目录的旧 `vibe coding/` 仅是历史基线，不能与当前实现混用，也不能作为 Day 3/Day 4 的完成证据。
- Day 3 双轨校准已经形成：CMP 完成 strict 执行但 architecture FAIL，并被阻断；独立 fresh S1 完成 strict PASS、Leaf STOP、真实 Coding 和 public pytest。S1 首次代码测试通过，**repair 次数为 0**。
- 因此，Day 3 的 repair 要求改为独立的 deterministic 初始失败 fixture：它必须真实运行 pytest → repair → pytest，并保存 failure evidence、patch、hash 与最终测试结果；不得人为破坏已经通过的 S1 模型输出来制造 repair。
- Day 4 已出现有效的 fresh root 证据：首个 run 在 PRD 阶段发生系统 ERROR，后续 run 完成 PRD、Architecture 和 Gherkin 后在 Mocktest semantic gate 得到 FAIL。该 FAIL 触发的正确动作是根据报告**定向修复 Architecture 并重新验证**，而不是进入 Leaf Gate 或 Coding。
- Day 4 Gate 仍为 **NO-GO / in progress**：只有获得 `root CONTINUE_LAYERING → fresh child STOP_LAYERING`、通过验证的 Leaf/Coding/repair 证据及相应验收测试后，才可转入 Day 5。
- Day 6 freeze 前必须新增并冻结 `RQ × C0–C5 × primary/secondary metrics × raw evidence path × figure/table` 登记表，以及脱敏证据索引。原始证据可私有保存，但共享归档必须含相对路径、hash、版本与结论摘要。
