...
class FrameworkConfig(BaseModel):
    ...
    name: str = Field(default="Mock测试框架")
...
class ExecutionConfig(BaseModel):
    ...
    mode: str = Field(default="skill", description="skill | api")


class SkillConfig(BaseModel):
    """Skill 模式配置"""

    max_modify_rounds: int = Field(default=3)
    auto_modify: bool = Field(default=True)
    pause_on_high_severity: bool = Field(default=False)
    show_reasoning: bool = Field(default=True)
    show_modification_diff: bool = Field(default=True)
    enable_cross_layer_check: bool = Field(default=True)
    enable_modification_history: bool = Field(default=True)
    max_history_versions: int = Field(default=10)
    cache_file_path: str = Field(
        default=".claude/skills/validate-arch/.tmp/prompt_cache.json",
...
class PbtConfig(BaseModel):
    ...
    max_examples: int = Field(default=100)
    default_mode: str = Field(default="both")
    timeout_seconds: int = Field(default=30)
    report_format: str = Field(default="markdown")
    layer_aware: bool = Field(default=True)
    pbt_mode: Literal["auto", "force", "skip"] = Field(
        default="auto", description="auto | force | skip"
    )
