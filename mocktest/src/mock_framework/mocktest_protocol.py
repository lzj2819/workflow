"""Versioned delivery contract for strict validate-arch runs.
...
class FormalInputManifest(StrictModel):
    schema_version: Literal["mocktest-input/v1"] = "mocktest-input/v1"
    run_id: str = Field(min_length=1)
    project_id: str = Field(min_length=1)
    node_id: str = Field(min_length=1)
    parent_node_id: str | None = None
    ...
    def validate_branches(self) -> "FormalInputManifest":
        ...
        for name, artifact in (("architecture", self.architecture), ("testcases", self.testcases)):
...
def write_schemas(output_dir: str | Path) -> None:
    ...
    schemas: tuple[tuple[str, type[BaseModel]], ...] = (
        ("mocktest_input.schema.json", FormalInputManifest),
        ("architecture.schema.json", NormalizedArchitecture),
        ("testcases.schema.json", NormalizedTestcases),
        ("mocktest_report.schema.json", MocktestReport),
        ("leaf_gate_evidence.schema.json", LeafGateEvidence),
        ("execution_log.schema.json", ExecutionLog),
    )
