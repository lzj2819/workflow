...
Usage:
  ...
  # 1. 准备：组件卡片 + 各场景逐跳执行计划
  python run_subagent_skill.py prepare --feature testcases.json --arch architecture.json --output plan.json
...
def _artifact_version(path: str | Path) -> str:
    ...
    try:
        ...
        return ""
    return str(value.get("artifact_schema_version") or value.get("schema_version") or "")
...
def cmd_prepare(args: argparse.Namespace) -> int:
    ...
    feature_projection_path: Path | None = None
    if Path(feature_path).suffix.lower() == ".feature" and _artifact_version(arch_path) == "architecture/v2":
        try:
            testcase_authority, feature_projection_path = resolve_testcases_authority(feature_path)
            feature_path = str(testcase_authority)
        except (OSError, ValueError, KeyError, TypeError, json.JSONDecodeError) as exc:
            print(f"ERROR: canonical Feature v2 adapter rejected input: {exc}", file=sys.stderr)
            return 5
    arch_version = _artifact_version(arch_path)
    testcase_version = _artifact_version(feature_path)
    canonical_v2 = arch_version == "architecture/v2" and testcase_version == "testcases/v2"
    if (arch_version == "architecture/v2") != (testcase_version == "testcases/v2"):
        print(
            "ERROR: canonical v2 input requires architecture/v2 and testcases/v2 together",
            file=sys.stderr,
        )
        return 5
    if canonical_v2 and getattr(args, "entry_overrides", None):
        print("ERROR: canonical v2 bindings cannot be overridden", file=sys.stderr)
        return 5

    # Aggregate legacy Markdown architecture docs into a run-scoped file.
    arch_input_path = arch_path
    if not canonical_v2 and Path(arch_path).is_dir():
        arch_input_path = skill._aggregate_arch_docs(arch_path, run_inputs_dir)
        print(f"Aggregated arch docs to: {arch_input_path}")

    # Canonical v2 is already scoped and must never pass through Markdown
    # rewrite/heading inference.  Only the explicit legacy adapter preprocesses.
    if not canonical_v2:
    ...
    print(f"Architecture adapter input: {arch_input_path}")
    ...
    if canonical_v2:
        try:
            loaded = load_canonical_pair(
                arch_input_path,
                feature_path,
                feature_projection_path=feature_projection_path,
            )
        except (OSError, ValueError, KeyError, TypeError, json.JSONDecodeError) as exc:
            print(f"ERROR: canonical v2 adapter rejected input: {exc}", file=sys.stderr)
            return 5
        initialize_workspace(
            output_path.parent,
            loaded.normalized_input,
            loaded.extraction_report,
            run_id=(formal_manifest.run_id if formal_manifest else run_manifest["run_id"]),
        )
        feature_files: list[Path] = []
    else:
        feature_files = _feature_files(feature_path)
        loaded = loader.load(str(feature_files[0]), arch_input_path)
    if not canonical_v2 and len(feature_files) > 1:
        merged_test_cases = []
    ...
    arch_doc = loaded.arch_doc if canonical_v2 else _parse_arch_doc(arch_input_path)
    ...
    for tc in loaded.test_cases:
        ...
        canonical_binding = loaded.bindings.get(tc.test_case_id) if canonical_v2 else None
        if canonical_binding and canonical_binding["status"] == "BOUND":
            selected_binding = canonical_binding["selected"]
            entry = {
                "entry_component": selected_binding["component_id"],
                "entry_action": selected_binding["action"],
                "confidence": "high",
                "reason": "canonical requirement-to-runtime-flow binding",
            }
        elif canonical_v2:
            entry = {
                "entry_component": "",
                "entry_action": "",
                "confidence": "blocked",
                "reason": f"canonical binding is {canonical_binding['status'] if canonical_binding else 'UNBOUND'}",
            }
        else:
        ...
        for interaction in interactions:
            ...
            if canonical_binding and canonical_binding["status"] == "BOUND":
                interaction["entry_contract_id"] = canonical_binding["selected"]["contract_id"]
                interaction["entry_contract_status"] = "resolved"
                interaction["entry_binding_kind"] = "canonical_runtime_flow"
                interaction["entry_contract_candidates"] = [canonical_binding["selected"]["contract_id"]]
        ...
        if canonical_binding and canonical_binding["status"] == "BOUND":
            plan_item["entry_contract_id"] = canonical_binding["selected"]["contract_id"]
            plan_item["entry_contract_status"] = "resolved"
            plan_item["entry_contract_reason"] = "canonical runtime flow"
            plan_item["entry_binding_kind"] = "canonical_runtime_flow"
            plan_item["entry_contract_candidates"] = [canonical_binding["selected"]["contract_id"]]
    ...
    output_data = {
        ...
        "entry_inference": "canonical-runtime-flow" if canonical_v2 else "legacy-local-architecture",
    ...
    if canonical_v2:
        testcase_by_id = {
            str(item["tc_id"]): item for item in loaded.normalized_input["testcases"]
        }
        write_contract_json(
            output_path.parent / "execution_plan.json",
            {
                "schema_version": "1.0",
                "artifact_schema_version": "mocktest-plan/v2",
                "input_fingerprint": loaded.normalized_input["input_fingerprint"],
                "scenarios": [
                    {
                        "tc_id": plan["test_case_id"],
                        "scenario_id": testcase_by_id[plan["test_case_id"]]["scenario_id"],
                        "requirement_ids": testcase_by_id[plan["test_case_id"]]["requirement_ids"],
                        "binding": loaded.bindings[plan["test_case_id"]],
                        "steps": testcase_by_id[plan["test_case_id"]]["steps"],
                        "interaction_count": len(plan.get("interaction_sequence") or []),
                        "state": (
                            "READY"
                            if loaded.bindings[plan["test_case_id"]]["status"] == "BOUND"
                            else "BLOCKED"
                        ),
                    }
                    for plan in plans
...
def cmd_run_strict(args: argparse.Namespace) -> int:
    ...
    canonical_input = _read_json(output_dir / "normalized_input.json", {})
    if canonical_input.get("artifact_schema_version") == "mocktest-normalized-input/v2":
        publish_canonical_bundle(output_dir, formal_output_dir)
        canonical_report = _read_json(formal_output_dir / "mocktest_report.json", {})
        canonical_status = canonical_report.get("states", {}).get("overall", "ERROR")
        formal_report = None
    else:
        ...
        canonical_status = formal_report.status
    summary = _read_json(summary_path, {})
    summary["mocktest_status"] = canonical_status
    ...
    return 0 if canonical_status == "PASS" else (2 if canonical_status in {"FAIL", "WARNING", "BLOCKED"} else 4)


def cmd_publish_artifacts(args: argparse.Namespace) -> int:
    canonical_input = _read_json(Path(args.run_dir) / "normalized_input.json", {})
    if canonical_input.get("artifact_schema_version") == "mocktest-normalized-input/v2":
        publish_canonical_bundle(args.run_dir, args.output_dir)
        report = _read_json(Path(args.output_dir) / "mocktest_report.json", {})
        print(json.dumps(report, ensure_ascii=False, sort_keys=True, indent=2), flush=True)
        status = report.get("states", {}).get("overall", "ERROR")
