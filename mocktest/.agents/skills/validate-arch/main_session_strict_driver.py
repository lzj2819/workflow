...
This helper keeps the original batch/multi-session flow intact. It supports an
alternate execution strategy for environments where nested subagents are not
available: the parent session calls this script to prepare prompt files, then
spawns each component/validator subagent directly and asks it to write the raw
JSON response file.
"""
