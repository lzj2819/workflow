    ...
    for line in content.splitlines():
        ...
        if last_requirement is not None and line[:1].isspace() and metadata_match:
            ...
            if key in {"implementation_surfaces", "related_reqs", "evidence_refs"}:
                ...
                last_requirement[key] = None if value.casefold() in {"n/a", "null", "none", "-"} else value
            continue
...
def _parse_structured_requirements(content: str) -> dict[str, list[dict[str, Any]]]:
    ...
    for raw_line in content.splitlines():
        ...
        if re.match(r"^#\s+(?:\d+\.\s+)?Current Release.*Functional Requirements", stripped, re.IGNORECASE):
            in_functional_section = True
            in_nfr_section = False
            in_exclusion_section = False
            current = None
            continue
        if re.match(r"^#\s+(?:\d+\.\s+)?Current Release.*Non-functional Requirements", stripped, re.IGNORECASE):
            in_functional_section = False
            in_nfr_section = True
            in_exclusion_section = False
            current = None
            continue
        if re.match(r"^#\s+(?:\d+\.\s+)?Future Backlog", stripped, re.IGNORECASE):
            in_functional_section = False
        ...
        if stripped.startswith("# "):
            in_functional_section = False
            in_nfr_section = False
            if not re.match(r"^#\s+(?:\d+\.\s+)?Future Backlog", stripped, re.IGNORECASE):
                in_exclusion_section = False
...
def _parse_acceptance_contracts(content: str) -> list[dict[str, Any]]:
    ...
    heading = re.search(r"^#\s+(?:\d+\.\s+)?Acceptance Contracts\s*$", content, re.MULTILINE | re.IGNORECASE)
    if not heading:
        return []
...
def _parse_success_metrics(content: str) -> list[dict[str, str]]:
    heading = re.search(r"^#\s+(?:\d+\.\s+)?Success Metrics\s*$", content, re.MULTILINE | re.IGNORECASE)
    if not heading:
        return []
    next_section = re.search(r"^#\s+", content[heading.end():], re.MULTILINE)
    end = heading.end() + next_section.start() if next_section else len(content)
